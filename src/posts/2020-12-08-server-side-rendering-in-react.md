...
SSR - это популярная техника для отрисовки *client-side* одностраничных приложений (SPA) на стороне сервера и последующей отправкой полностью отрендереной страницы клиенту. Подход полезен для SEO (поисковые боты до сих пор не умеет правильно обрабатывать JavaScript), UX (пользователи сразу получают отрендеренные страницы) и лучших показателей метрик производительности.

Уже существует немало решений с поддержкой SSR из коробки (тот же [Next.js](https://nextjs.org/)), но понимание, как все работает изнутри и тонкая настройка под себя бывают просто необходимо при работе с серверным рендерингом.
...
Работать будем над самым типичным приложением для React-экосистемы. Но искусственные проекты без темы - скучные, поэтому сегодня будет готовить каталог кроссовок с главной страницей, каталогом и карточкой товара.
...
Вы могли заметить, что нет директории `containers`. Приложение небольшое, логика для связи редакса и реактовских компонентов помещена в сами компоненты. Например, вот выглядит компонент главной страницы (`Home.tsx`) - берем экшен, данные из селекторов и пробрасываем в компонент:
...
Также в проекте не используются реальные API и все данные заранее подготовлены. Но при их получении используется задержка в 500мс для эмуляции реальной загрузки с внешнего ресурса:
...
Так же запускать сервер нужно самим, как и перезапускать при изменении бандла для режима разработки. Посмотрим на схему запуска проекта с SSR:
...
- `webpack-node-externals` - Плагин для вебпака для серверной сборки
- `@types/*` - TS-тайпинги
...
> Webpack будет собирать код параллельно для двух сред, еще и Nodemon будет перезапускать сервер, поэтому для понимания, что сейчас собирается, очень помогает опция `--print-label` для `npm-run-all`, которая будет выводить лейбл в каждой строке терминала:
...
> Замечу, что это не применимо для CSS-Modules или Styled-Components, только для обычного CSS.
...
Начнем, сначала немного освежим файл `src/store/rootStore.ts`, в котором инициализируется *Redux store*. Создадим хелпер `isServer`:
...
> Если данные в стейте могут задавать пользователи (*UGC*), формируется из внешних API, то возможна XSS уязвимость. В таких случаях данные нужно проверять и как минимум экранизировать.
...
Сперва разберемся с сагами. Вы можете пропустить этот раздел, если используете что-то другое. Саги, построенные на генераторах, нужны для работы с асинхронными вещами, с которыми обычный редакс не работает. Одни из самых частых асинхронных операций в приложении - это отправка запросов в API за данными - как раз наш случай.
...
Для остановки саги есть специальный экшен `END`. Самое классное, что он сперва обработает все запущенные саги, и только после заканчивает выполнение. Процесс работы выглядит следующим образом: запускаем сагу на сервере, скармливаем ей экшены, после диспатчим `END` и ждем, пока сага завершит выполнение. Посмотрим, как будет выглядеть в коде.

Сначала добавим тип `AppStore`, в котором опишем два метода (для запуска и остановки саги):
...
Сначала вынесем роуты в отдельный файл. Сравнивая path с текущим адресом страницы мы сможем определить нужный компонент, который необходимо отрендерить:
...
Без SSR следующая картина: пользователь получает быстрее html (но пустую), потом инициализирует реакт и видит заглушки (какой-нибудь лоадер в общем случае) и только после получения данных получает страницу с контентом:
...
Для ее решения нужно разбить бандл на несколько небольших. _Code splitting_ (дословно, _разделение кода_) - это как раз про это. Когда пользователь запрашивает страницу, браузер будет загружать только нужные части, а остальные подгружать по необходимости, тем самым можно очень сильно уменьшить первоначальный размер загружаемых ресурсов.
...
На одном из рабочих проектов мы решили не внедрять _Code splitting_, потому что проект был небольшой. Но использовали [DLL плагин](https://webpack.js.org/plugins/dll-plugin/), который позволяет вынести внешние зависимости (типа, _React_, список нужно указать самому) в отдельный бандл (и очень просто интегрируется). 

Почти бесплатно получаем профит:
...
На рабочем проекте получилось два бандлы (vendor бандл и с кодом проекта)и выглядят они вот так:
...
`chunkExtractor` будет содержать всю информацию о js и ccs-файлах, которые необходимо подключить. Поэтому вместо явного указания файлов, используем информацию их этой переменной:
