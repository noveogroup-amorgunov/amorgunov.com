...
Но как быть, например, с модалками, когда мы хотим показать анимацию ухода, но при закрытии она моментально уходит из DOM? Необходимо перед закрытием делать задержку на N времени, показывать анимацию и потом "убивать" компонент.
...
Но если мы хотим изменить высоту, ширину или другие свойства (кроме трансформации и непрозрачности), но анимация на css обычно получается не совсем такая, какую мы хотим получить - немного дерганная.
...
- **Last**: выполнить код, который вызывает мгновенный переход на конечное положение и сохраняет позицию и размеры элемента.
...
Эта техника идеально подходит, когда нам нужно реагировать на действия пользователя и потом что-то анимировать. Например, есть галерея изображений, и при клике на изображение мы увеличиваем его. Часто начальные и конечные положение элементов не известны, и сделав расчеты заранее, получится поддерживать анимацию c 60fps. Одно *но*, эти расчеты должны уложится в 100мс после действия пользователя, которые пользователь не сможет заметить, и ему будет казаться, что сайт отреагировал мгновенно.
...
Сейчас для анимации можно использовать [полифил the Web Animations API](https://github.com/web-animations/web-animations-js). Еще один приятным плюсом является то, что финальным элемент может не совпадать с начальным.
...
Если у нас есть большая фотография (более оптимально использовать jpg), и нужно сделать прозрачность, то придется использовать png (весить изображение будет довольно много). Но можно разделить изображения на два, прозрачную часть сделать в png, а фото - jpg. Фотку можно обрезать и использовать [svg маску](https://medium.com/zappos-engineering/image-masking-with-svg-addfd2726753), которая будет указывать видимую часть изображения. В качество фона поставить полупрозрачную png. Этим способом можно добиться неплохих результат по уменьшению размера, но максу придется вырезать вручную, и это не так-то просто. Поэтому этот способ можно использовать только в крайних случаях.

---

## Работа с AST

Последний доклад был немного хардкорный, но не менее интересный, Никита Сидоров рассказал про e2e тестирование, а именно как в selenium-подобных системах выбирать нужный элемент, и как ребята в Яндекс.Маркете с помощью парсинга AST (абстрактного синтаксического дерева) добавляли каждому компоненту уникальный "data-атрибут".
...
В общем ходите на митапы, а если что рассказать, то не в качестве слушателя, а в качестве докладчика. Всем бобра
