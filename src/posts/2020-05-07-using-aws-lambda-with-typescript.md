...
- TypeScript позволяет собирать код под разные ecmascript стандарты (например, можно собрать код под es5 c var-ами). Nodejs v12 полностью поддерживает `es2019` (подробнее можно почитать здесь: https://stackoverflow.com/questions/59787574/typescript-tsconfig-settings-for-node-js-12), поэтому `target` в конфиге указан с соответствующим значением.
- Опция lib говорит TypeScript, что в платформе реализованы указанные фичи и не нужно их дополнительно преобразовывать.
...
> Рекомендую почитать материал "[Как писать тесты в Nodejs](http://localhost:8080/posts/2020-01-28-how-write-tests-in-nodejs/)" о правильных практиках и подходах по написанию тестов.
