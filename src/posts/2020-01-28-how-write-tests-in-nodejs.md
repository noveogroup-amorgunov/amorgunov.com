...
2. Складывать тест рядом с модулем. В таком случае нет проблем искать файл, он всегда лежит рядом, но такие файлы они создают некий шум, и становится сложнее искать модули, потому что количество файлов в рамках одной директории возрастает в два раза;
...
- Слушать сервер на каком-то порту (app.listen) нужно вынести в отдельный файл, чтобы в `app.js` только навешивались роуты и миддлевары на приложение;
- Для отправки запросов из тестов будем использовать библиотеку [supertest](https://github.com/visionmedia/supertest), которая позволяет тестировать веб-серверы в декларативном подходе.
...
> ✒️ Нужно не забыть написать тест на случай, если внешний API недоступен, и в случае подобной ситуации, ваше приложение уже будет готово к такой ситуации.
...
Поэтому на всех следующих проектах мы делали тесты независимыми путем удаления всех данных из базы данных. Тесты в таком случае независимы друг от друга, но общее время выполнения возрастает.
