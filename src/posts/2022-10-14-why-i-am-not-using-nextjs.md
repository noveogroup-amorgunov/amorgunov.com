...
Положительные стороны конечно же есть: очень много проблем решено из коробки, с которыми пришлось бы работать вручную без использования NextJS. Выходит много новых фич (та же возможность переключить _webpack_ под коробкой на другой компилятор типа _swc_), исправляются баги, [очередная конференция](https://nextjs.org/conf) через неделю будет (если конечно вы читаете этот пост не через пару лет после его написания). Или те же решения, о который я говорил в материале, _next-transpile-modules_, _next-redux-wrapper_ - их авторы просто красавчики, что сделали за нас огромную работу, которая позволяет использовать нам NextJS.
