...
function LayoutHeader() {
  ...
  const popularProducts = useSelector(
    state: State => state.popularProducts.items
//                        ^^^^^^^^^^^^^^^^^^^^^
  )
...
class LayoutHeaderService {
  constructor(
    public popularProducts: PopularProductsService
  ) {}
}
```
