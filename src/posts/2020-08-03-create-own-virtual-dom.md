...
likes: 2
---
Всем привет! Сегодня нас ждет удивительное приключение по реализации виртуального DOM-a с нуля.

Материал будет довольно большим, чтение у вас может занять до получаса, имейте это ввиду. А если у вас нет столько времени, то вы можете сразу посмотреть результат в песочнице [codesandbox](https://codesandbox.io/s/vdom130-12fsf) или [на гитхабе](https://github.com/noveogroup-amorgunov/vdom130).
...
Такой паттерн позволяет описывать интерфейсы в декларативном подходе, абстрагирует нас от прямой работы с DOM, обработчиками событий и атрибутов, а самое важное: оптимизирует работу обновления только нужных частей DOM-дерева (отвечает за синхронизацию с реальным DOM).

На медиуме есть [классный пост](https://medium.com/@nickbulljs/how-virtual-dom-work-567128ed77e9), как работает VDOM в React-е, а мы далее реализуем его сами (конечно очень упрощенную, но рабочую версию). Приступим!
...
Мы подошли к самому интересному - сравнение виртуальных нод и применение изменений к реальной DOM-ноде. Идея в следующем:
 ...
 - Строится виртуальное дерево, по которому строится реальное и монтируется в DOM;
 ...
 - Текущий виртуальным DOM заменяем новым.
...
setInterval(() => {
  ...
  // Текущие виртуальное дерево заменяем новым
  vApp = nextVApp;
}, 1000);
```
...
Обновление дочерних нод - процесс немного сложнее, но без какой либо магии и всего в несколько строк кода. В метод нужно передать родительскую ноду, так как именно ее детей необходимо править (удалять, добавлять и обновлять). Рассмотрим возможные кейсы:
...
Что же, мы решили проблемы и сейчас обновляются только измененные элементы! Но если мы посмотрим на код в `app.js`, то он не сильно удобный для использования (постоянно хранить ссылку как и DOM-элемент, так и на виртуальный DOM, который еще нужно и заменять). Инкапсулируем всю эту логику в методе `patch`.
...
const recycleNode = node => {
  // Если тесктовая нода - то возвращаем текст
